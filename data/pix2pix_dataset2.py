...
class Pix2pixDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        #label = np.array(label) / IMG_DIV
        #print(label.shape) # (512, 512, 3)

        params = get_params(self.opt, (256, 256))
        #transform_label = get_transform(self.opt, params, method=Image.NEAREST, normalize=False)

        #label = label.convert('RGB')
        transform_image = get_transform(self.opt, params, RGB=False)
        ...
        #image = np.array(image) / IMG_DIV

        #image = image.convert('RGB')
        transform_image = get_transform(self.opt, params, RGB=True)
